...
-- Function to get organization_id from JWT (in public schema)
...
-- Function to check if user is admin (in public schema)
...
CREATE POLICY "Organizations visible to members" ON organizations
  FOR SELECT USING (
    id = public.get_organization_id()
  );

CREATE POLICY "Organizations updateable by admins" ON organizations
  FOR UPDATE USING (
    id = public.get_organization_id() AND public.is_admin()
  );
...
CREATE POLICY "Users insertable by admins" ON users
  FOR INSERT WITH CHECK (
    organization_id = public.get_organization_id() AND public.is_admin()
  );
...
CREATE POLICY "Channel agents visible to organization members" ON channel_agents
  FOR SELECT USING (
    EXISTS (
      ...
      AND channels.organization_id = public.get_organization_id()
    )
  );
